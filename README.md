...
Variable	Purpose
OPENAI_API_KEY  AI digests + email curation
MY_EMAIL  Your Gmail address
APP_PASSWORD  16-char Gmail app password (not normal password)
POSTGRES_*  Database (defaults usually fine)
...
Step	Command
Scrape OpenAI RSS  .venv/bin/python app/scrapers/openai.py
Scrape Anthropic  .venv/bin/python app/scrapers/anthropic.py
Generate digests  .venv/bin/python -m app.services.process_digest
Send email digest  .venv/bin/python -m app.services.process_email
Test email only  .venv/bin/python app/services/email_delivery.py

5. What “success” looks like

The pipeline logs progress like:
a. Scraping (YouTube, OpenAI, Anthropic)
b. Processing Anthropic markdown
c. YouTube transcripts
d. Creating digests (OpenAI API — watch for 429 rate limits)
e. Sending email digest
